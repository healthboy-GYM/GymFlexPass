...
import { NAVER_CTS_ACCOUNT_ID } from '@/lib/naverCts';
...
export function TrackingScripts() {
  useEffect(() => {
    ...
    const initNaver = () => {
      ...
      window.wcs.inflow();
      window.wcs_do();
      // 2) 네이버 CTS(전환추적) 유입 — AccountId 가 설정된 경우에만
      if (NAVER_CTS_ACCOUNT_ID) {
        window.wcs_add['wa'] = NAVER_CTS_ACCOUNT_ID;
        window.wcs.inflow('healthboypass.co.kr');
        window.wcs_do();
      }
    };
    if (window.wcs) {
