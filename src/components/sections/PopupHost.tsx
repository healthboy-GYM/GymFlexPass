...
/** 노출 기간(시작일~종료일) 안에 있는지. 날짜 문자열(YYYY-MM-DD) 사전식 비교. */
function isWithinSchedule(item: PopupItem): boolean {
  const today = todayStr();
  if (item.startDate && today < item.startDate) return false; // 시작 전
  if (item.endDate && today > item.endDate) return false;     // 종료 후
  return true;
}
...
function isEligible(item: PopupItem): boolean {
  ...
  if (!isWithinSchedule(item)) return false;
