...
import { Lightbulb, BarChart3, AlertTriangle, CheckCircle, Crown, Gem, Medal, Heart, X, LocateFixed, Loader2, Info, XCircle, Search, CircleHelp, ShoppingCart, Percent, Clock, Car, ArrowLeft, Rocket, Megaphone, Phone, MapPin, ArrowRight } from 'lucide-react';
import { getBranchMeta, REGION_SLUG } from '@/lib/branchMeta';
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from '@/components/ui/accordion';
import { ScrollArea, ScrollBar } from '@/components/ui/scroll-area';
import { useGyms } from '@/hooks/use-gyms';
import { recommendGymPass, type GymRecommendationOutput } from '@/lib/recommend';
...
const GymInfoCard = ({ gym, onSelectToggle, onDeselect, isSelected, videoUrl, videoError }: { gym: Gym; onSelectToggle: (id: string) => void; onDeselect: () => void; isSelected: boolean; videoUrl: string | null; videoError: string | null; }) => {
    ...
    const parkingInfo = (gym['무료주차시간'] && gym['무료주차시간'] !== 'X')
        ? `무료주차: ${gym['무료주차시간']}`
        : '유료 주차';
    const meta = getBranchMeta(gym.name);
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 p-2 md:p-4 animate-in slide-in-from-bottom-10 duration-300">
            <Card className="w-full max-w-7xl mx-auto shadow-2xl border-t-2 border-primary overflow-hidden">
                ...
                <div className="flex flex-row">
                    ...
                    <div className="w-2/3 p-4 space-y-2.5">
                        ...
                        {gym.transport && (
                            <p className="flex items-start gap-1.5 text-xs text-muted-foreground">
                                ...
                                <span className="line-clamp-2">{gym.transport}</span>
                            </p>
                        )}
                        {facilities.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                                {facilities.slice(0, 5).map((f) => (
                                    <span key={f} className="rounded-full bg-secondary px-2 py-0.5 text-[11px] leading-tight text-foreground">{f}</span>
                                ))}
                            </div>
                        )}
                         <div className="flex flex-col sm:flex-row gap-2 pt-1">
                           ...
                           {slug && (
                               <Button asChild variant="outline" className="w-full">
                                   <Link href={`/branch/${slug}`}>지점 상세 보기 <ArrowRight className="ml-1 h-4 w-4" /></Link>
                               </Button>
                           )}
                           {gym.phone && (
                               <Button asChild variant="outline" className="w-full sm:w-auto">
                                   <a href={`tel:${gym.phone.replace(/[^0-9]/g, '')}`} aria-label="전화하기"><Phone className="h-4 w-4 sm:mr-0" /><span className="sm:hidden ml-2">전화</span></a>
                               </Button>
                           )}
                        </div>
                    </div>
...
export function BranchLocator({ hideNavigation = false }: { hideNavigation?: boolean }) {
  ...
  const { listTitle, listData } = useMemo(() => {
    ...
    if (selectedRegion !== 'all' && !searchTerm.trim() && selectedTier === 'all') {
        title = `${shortenRegionName(selectedRegion)} 지점`;
    } else if (selectedRegion !== 'all' || searchTerm.trim() || selectedTier !== 'all') {
        title = '검색 결과';
    }
  ...
  useEffect(() => {
    if (selectedGymIds.size === 0) {
      ...
      const getRecommendation = async () => {
        try {
          const result = recommendGymPass({ selectedGymIds: Array.from(selectedGymIds) }, allGyms as any);
        ...
        <div className="container px-4 md:px-6 animate-in fade-in-0 slide-in-from-bottom-5 duration-1000 ease-in-out">
            ...
            <Accordion type="single" collapsible className="w-full" defaultValue="gym-list">
                ...
                <div className="flex items-center justify-between gap-3">
                    <AccordionTrigger className="flex-1 text-xl font-semibold hover:no-underline" data-gtm-id="branch-locator-gym-list-accordion-click">{listTitle}</AccordionTrigger>
                    {selectedRegion !== 'all' && REGION_SLUG[selectedRegion] && !searchTerm.trim() && selectedTier === 'all' && nearbyGyms.length === 0 && (
                        <Button asChild variant="link" size="sm" className="shrink-0 text-primary font-medium">
                            <Link href={`/area/${REGION_SLUG[selectedRegion]}`} data-gtm-id="branch-locator-region-landing-click">
                                {shortenRegionName(selectedRegion)} 지점 전체 안내 <ArrowRight className="ml-1 h-4 w-4" />
                            </Link>
                        </Button>
                    )}
                </div>
