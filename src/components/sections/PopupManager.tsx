...
import { Loader2, Save, Plus, Trash2, Eye, ChevronUp, ChevronDown, Upload } from 'lucide-react';
...
export function PopupManager() {
  ...
  const handleImageUpload = (id: string, file: File) => {
    ...
    task.on(
      ...
      () => {
        patch(id, { imageUrl: publicAssetUrl(file.name) } as Partial<PopupItem>);
        setUploadingId(null);
  ...
  const handleSave = async () => {
    ...
    for (const it of items) {
      if (it.startDate && it.endDate && it.startDate > it.endDate) {
        toast({ variant: 'destructive', title: '노출 기간 오류', description: `${KIND_LABEL[it.kind]}: 시작일이 종료일보다 늦습니다.` });
        return;
      }
      if (it.kind === 'custom') {
        if (it.enabled && !it.title.trim()) { toast({ variant: 'destructive', title: '제목 필요', description: '켜져 있는 커스텀 팝업에는 제목이 필요합니다.' }); return; }
        if (it.buttonLink?.trim() && !/^(https?:\/\/|\/)/.test(it.buttonLink.trim())) { toast({ variant: 'destructive', title: '링크 오류', description: '버튼 링크는 http(s):// 또는 내부 경로(/purchase 등)여야 합니다.' }); return; }
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            <Input className="flex-1" value={c.imageUrl ?? ''} onChange={(ev) => patch(it.id, { imageUrl: ev.target.value } as Partial<PopupItem>)} placeholder="이미지 업로드 또는 URL" />
            <Button
              type="button" variant="outline" className="shrink-0"
              disabled={uploadingId === it.id}
              onClick={() => { uploadTargetId.current = it.id; fileInputRef.current?.click(); }}
            >
              {uploadingId === it.id ? <><Loader2 className="mr-1.5 h-4 w-4 animate-spin" />{uploadPct}%</> : <><Upload className="mr-1.5 h-4 w-4" />업로드</>}
            </Button>
          </div>
          {c.imageUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={c.imageUrl} alt="" className="mt-1 h-24 w-full rounded-md border object-cover" />
          ) : null}
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
    ...
    <div className="space-y-6">
      ...
      {items.map((it, index) => (
        <div key={it.id} className="space-y-3 rounded-xl border bg-card p-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">노출 시작일 (선택)</Label>
              <Input type="date" value={it.startDate ?? ''} onChange={(e) => patch(it.id, { startDate: e.target.value } as Partial<PopupItem>)} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">노출 종료일 (선택)</Label>
              <Input type="date" value={it.endDate ?? ''} onChange={(e) => patch(it.id, { endDate: e.target.value } as Partial<PopupItem>)} />
            </div>
          </div>
          <p className="-mt-1 text-[11px] text-muted-foreground">비워두면 상시 노출. 기간을 정하면 시작일~종료일(당일 포함)에만 노출됩니다.</p>
