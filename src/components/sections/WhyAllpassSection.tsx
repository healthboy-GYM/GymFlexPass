  ...
  return (
    ...
    >
      <div className="container px-4 md:px-6 animate-in fade-in-0 slide-in-from-bottom-5 duration-1000 ease-in-out">
        <Reveal className="flex flex-col items-center justify-center space-y-3 text-center mb-12">
          <span className="inline-flex items-center gap-1.5 text-xs font-bold tracking-[0.16em] text-primary uppercase">
            01 · Your Problem
          </span>
          <h2 className="text-4xl font-black tracking-tighter sm:text-5xl md:text-6xl leading-[1.05]">
            당신의 운동, <br className="sm:hidden" />혹시 이런 <span className="text-primary">고민</span><br className="hidden sm:block" /> 없으셨나요?
          </h2>
        </Reveal>
        <div className="mx-auto grid max-w-5xl items-start gap-6 sm:grid-cols-1 md:grid-cols-3 md:gap-8">
          {problems.map((problem, index) => (
            ...
            <Card
              className="relative overflow-hidden rounded-2xl shadow-lg group h-80 md:h-96 border-border/50 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/20"
            >
              <Image
              ...
              <span className="absolute top-4 left-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-primary text-primary-foreground text-base font-black shadow-lg">
                {index + 1}
              </span>
              <CardContent className="relative h-full flex flex-col justify-end p-6">
                <span className="mb-3 h-1 w-10 rounded-full bg-primary transition-all duration-300 group-hover:w-16" />
                <p className="text-2xl font-bold leading-snug text-white transition-transform duration-300 group-hover:-translate-y-1">
                  {problem.text}
                </p>
              </CardContent>
            </Card>
