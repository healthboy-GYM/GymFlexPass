...
export function GymManager() {
  ...
  const handleImport = async () => {
    if (!confirm('기존 67개 지점을 Firestore로 가져올까요? (이미 데이터가 있으면 건너뜁니다.)')) return;
    setImporting(true);
    try {
    ...
    <div className="space-y-4">
      ...
      {gyms.length === 0 ? (
        <div className="rounded-md border bg-card p-10 text-center text-muted-foreground">
          아직 지점 데이터가 없습니다. <strong className="text-foreground">‘기존 67개 지점 가져오기’</strong>로 시작하거나 직접 추가하세요.
        </div>
      ) : (
