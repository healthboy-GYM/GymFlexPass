...
interface PopupBase {
  ...
  startDate?: string; // 노출 시작일(YYYY-MM-DD). 비우면 즉시. 이 날짜 이전에는 노출 안 함.
  endDate?: string;   // 노출 종료일(YYYY-MM-DD). 비우면 무기한. 이 날짜가 지나면 노출 안 함.
}
...
function normalizeItem(x: unknown): PopupItem | null {
  ...
  const base = {
    id: o.id,
    enabled: asBool((o as PopupBase).enabled),
    frequency: asFreq((o as PopupBase).frequency),
    startDate: asStr((o as PopupBase).startDate),
    endDate: asStr((o as PopupBase).endDate),
  };
