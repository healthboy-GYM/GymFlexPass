...
import { subscribePricing, DEFAULT_PRODUCTS, DURATIONS, DURATION_LABELS, type Product, type Duration } from '@/lib/pricing';
import { fireNaverConversion } from '@/lib/naverCts';
...
export default function PurchasePage() {
  ...
  // URL 쿼리로 상품/기간 사전선택: /purchase?product=<상품명 또는 id>&duration=1
  // (이벤트 팝업·랜딩의 '지금 시작하기'가 특정 상품이 선택된 구매 페이지로 보낼 때 사용)
  const preselectDone = useRef(false);
  useEffect(() => {
    if (preselectDone.current || products.length === 0 || typeof window === 'undefined') return;
    const params = new URLSearchParams(window.location.search);
    const want = (params.get('product') || params.get('p') || '').trim();
    const applyDuration = () => {
      const dur = (params.get('duration') || params.get('d') || '').trim();
      if (dur && (DURATIONS as readonly string[]).includes(dur)) setSelectedDuration(dur);
    };
    if (!want) { applyDuration(); preselectDone.current = true; return; }
    const norm = (s: string) => s.trim().toLowerCase();
    const match =
      products.find((p) => p.id === want) ||
      products.find((p) => norm(p.name) === norm(want)) ||
      products.find((p) => norm(p.name).includes(norm(want)));
    // 원하는 상품이 아직 로드되지 않았으면(초기 폴백 목록) done 처리하지 않고 다음 업데이트를 기다린다.
    if (!match) return;
  ...
  // 판매 중인 기간만 노출: 가격 > 0 이고 결제 링크가 있는 기간. (이벤트처럼 1개월만 파는 상품 지원)
  const availableDurations = useMemo<Duration[]>(() => {
    if (!selectedProduct) return [...DURATIONS];
    const offered = DURATIONS.filter((d) => {
      const e = selectedProduct.prices[d];
      return e && e.price > 0 && /^https?:\/\//.test((e.link ?? '').trim());
    });
    return offered.length > 0 ? offered : DURATIONS.filter((d) => (selectedProduct.prices[d]?.price ?? 0) > 0);
  }, [selectedProduct]);

  // 선택된 기간이 판매 목록에 없으면 첫 번째 판매 기간으로 자동 보정.
  useEffect(() => {
    if (availableDurations.length > 0 && !availableDurations.includes(selectedDuration as Duration)) {
  ...
  const handlePurchaseClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      fireNaverConversion({
        type: 'purchase',
        value: selectedPriceInfo.price,
        items: [{ id: selectedProduct.id, name: `${selectedProduct.name} ${selectedDuration}개월`, payAmount: selectedPriceInfo.price }],
      });
            ...
            <Card>
              ...
              <CardContent>
                <RadioGroup
                  ...
                  className={cn(
                    'grid gap-4',
                    availableDurations.length === 1 ? 'grid-cols-1' : availableDurations.length === 2 ? 'grid-cols-2' : 'grid-cols-3'
                  )}
                  ...
                  {availableDurations.map((value) => (
                      <Label
                        ...
                        className={cn(
                          ...
                          selectedDuration === value ? "border-primary ring-2 ring-primary bg-primary/5" : "border-border/50 hover:bg-secondary/50",
                          !selectedProduct && "cursor-not-allowed opacity-50"
                        )}
                        data-gtm-id={`purchase-select-duration-${value}-month-click`}
                      >
                        <span className="font-bold text-xl">{DURATION_LABELS[value]}</span>
                        <RadioGroupItem value={value} id={`duration-${value}`} className="sr-only" />
                      </Label>
                    )
                  )}
                </RadioGroup>
